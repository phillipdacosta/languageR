#!/usr/bin/env python3
"""Translate HOME keys that still match en.json (all locales)."""
...
SKIP_KEYS = frozenset({"PAYPAL_EMAIL_PLACEHOLDER"})
PH_RE = re.compile(r"\{\{[^}]+\}\}")
...
def main() -> None:
    ...
    en_home = json.loads((I18N_DIR / "en.json").read_text(encoding="utf-8"))["HOME"]
    ...
    for path in sorted(I18N_DIR.glob("*.json")):
        ...
        collect_stale(en_home, home, updates)
        if not updates:
        ...
        for i, (key_path, en_text) in enumerate(updates.items()):
            try:
                ...
                if translated and translated != en_text:
                    set_nested(home, key_path, translated)
                else:
