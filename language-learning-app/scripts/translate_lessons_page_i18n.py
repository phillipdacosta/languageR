#!/usr/bin/env python3
"""Translate LESSONS_PAGE strings missing or still identical to en.json."""
...
def collect_updates(en_branch: dict, loc_branch: dict, updates: dict[str, str]) -> None:
...
def apply_updates(branch: dict, en_branch: dict, flat: dict[str, str]) -> None:
    for k, ev in en_branch.items():
        if isinstance(ev, dict):
...
def process_locale(en: dict, path: Path) -> int:
    stem = path.stem
    if stem == "en":
        return 0
    ...
    en_lp = en.get("LESSONS_PAGE")
    if not isinstance(en_lp, dict):
        return 0

    if "LESSONS_PAGE" not in data or not isinstance(data["LESSONS_PAGE"], dict):
        data["LESSONS_PAGE"] = json.loads(json.dumps(en_lp))
    else:
        deep_merge_missing(data["LESSONS_PAGE"], en_lp)

    updates: dict[str, str] = {}
    collect_updates(en_lp, data["LESSONS_PAGE"], updates)
    if not updates:
        return 0

    keys = list(updates.keys())
    translated = translate_texts(GoogleTranslator(source="en", target=gt), [updates[k] for k in keys])
    apply_updates(data["LESSONS_PAGE"], en_lp, dict(zip(keys, translated)))
